...
> Detail-Baustand: `HANDOVER_transfer_2026-06-24.md`. §N = Abschnitt weiter unten in dieser Notiz.
...
- ⬜ **Modul-Editor** (Evolution; `aaUd`-Chunk walkbar, Donor-Splice; isoliert verifizieren) — §4.6/4.7
...
- ⬜ **Map-Knoten blockieren (jail-only):** reaktiver Eject (`on.room_change`→zurück+Notification), „freigeschaltet"
  = Storage-Flag → **progressiver Ausbau** (NICHT alle Knoten von Anfang an). Loop-Guard nötig. Vorbild Jailer. — §4.2/4.3/4.4
- ⬜ **jail = nur Nacht/late-night:** `on.period`-Hold (timewarp-Mechanik) + erzwungener Rückwechsel an der Tagesgrenze — §3.3/3.4
