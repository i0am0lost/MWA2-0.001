...
import resolver                # evolution brain: DBs -> active modules -> per-char value effects
...
# ----------------------------------------------------------------------------
DOTNET        = "dotnet"
PPEX_DLL      = _BASE + r"\system\ppex\PPeXM64\bin\Release\netcoreapp3.1\PPeXM64.dll"
MUTEXFIND_DLL = _BASE + r"\system\mutexfind\bin\Release\net8.0\mutexfind.dll"
...
JAIL_CELL     = 39       # the jail cell room (Outside Station). Every jail inmate is confined here.
...
_char_state_last = None


def _write_world_flag(filename, body):
    """Atomically write `body` to <filename> in every world dir (apply runs wherever a char is present)."""
    for w in WORLDS:
        path = os.path.join(w["dir"], filename)
        tmp = path + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            f.write(body)
        os.replace(tmp, path)
...
def write_char_state(con, ts):
    """Run the evolution resolver (DBs -> active modules per char) and write each char's resolved self-value
    EFFECTS for the apply-mod. Written to EVERY world dir (apply runs wherever a char is present). The card
    is NEVER written -- evolution lives in the SSOT, applied at runtime. Self-gated on content (no churn).
    Also emits the mwa_id-keyed reroute flag (the same resolution, keyed by stable id for the reroute hook)."""
    global _char_state_last
    resolved = resolver.resolve_all(con, ts)
    lines = []
    for name in sorted(resolved):
        # ONLY Card-Storage flags (@b:confined=1;@i:cell=...) -- the SSOT gates dispatcher modules via flags,
        # it never writes vanilla stats. char_id-keyed; apply_state matches by name, never by seat.
        eff = resolver.cardstorage_string(con, name)
        if eff:
            lines.append("%s\t%s\t%s" % (name, ",".join(resolved[name]), eff))
    body = ("# char_state (resolved): <name>\\t<active modules>\\t<card-storage flags>\n"
            + "\n".join(lines) + "\n")
    if body == _char_state_last:
        return resolved
    _char_state_last = body
    _write_world_flag("_orch_char_state.flag", body)
    return resolved
...
def transfer_sync(school_world):
    ...
    activity_snapshot_path = os.path.join(school_world["dir"], "_orch_activity_snapshot.flag")
    last = None
    while not _cleanup_done:
        try:
            ...
            if key and os.path.exists(twin_sav_path(key)):
                ...
                try:
                    ...
                    pdb.ingest_activity_snapshot(con, activity_snapshot_path, ts)  # cumulative H activity (notes 4.7)
                    # Jail inmates ARE confined to the cell -- that's what jail means. Auto-confine every
                    # current resident (char_id-keyed, one SSOT), and release anyone no longer in jail. This
                    # MUST run before write_char_state so the @b:confined/@i:cell flags land in char_state.
                    nday = read_day(school_world)
                    residents = pdb.residents_as_of(con, nday, "jail") if nday is not None else []
                    resident_ids = {r["char_id"] for r in residents}
                    for cid in resident_ids:
                        pdb.set_confine(con, cid, JAIL_CELL, ts)
                    for cid in list(pdb.all_confined(con)):
                        if cid not in resident_ids:
                            pdb.clear_confine(con, cid)        # left jail -> no longer confined

                    write_char_state(con, ts)   # resolve evolution -> per-char value effects + confine flags
                    if nday is not None:
...
def jail_ready_for_current(school_world):
    """True if jail is loaded for the save school currently has (safe/meaningful to switch).
    Module-level so both the GUI button and the headless console loop can use it."""
    jw = next((w for w in WORLDS if w.get("load_seq")), None)
    if not jw:
        return True
...
def switch_world(school_world):
    """Switch the active world to the next one (school <-> jail). Non-blocking: if the coupler is
    loading jail right now the request is ignored. Shared by the GUI button and the headless loop."""
    ...
    try:
        ...
        if tw.get("load_seq") and not jail_ready_for_current(school_world):
...
def run_headless(school_world):
    """Fallback when the Tk overlay can't start (e.g. a Python whose tkinter can't find Tcl). The
    worlds and all background threads keep running -- we only lose the on-screen label/button, NOT
    the orchestrator. ENTER switches the active world (school <-> jail); 'q'+ENTER or Ctrl+C quits
    (cleanup runs in main's finally). If there's no interactive console, we idle until shutdown."""
    while not _cleanup_done:
        try:
            line = input()
        except EOFError:
            while not _cleanup_done:        # no console attached -> just stay up, don't tear down
                time.sleep(1)
            return
        if line.strip().lower() in ("q", "quit", "exit"):
            return
        switch_world(school_world)
        print(f"[*] active world -> {_active_world}")
...
def run_ui(school_world):
    ...
    The background jail coupler keeps jail loaded, so switching is instant. If the Tk overlay can't
    start (broken tkinter/Tcl), we degrade to run_headless rather than tearing the worlds down."""
    try:
        root = tk.Tk()
    except tk.TclError as e:
        print(f"\n[!] GUI overlay unavailable: {e}")
        print("    -> HEADLESS mode: worlds stay up, jail still couples automatically.")
        print("       Press ENTER to switch the active world (school <-> jail), Ctrl+C to quit.\n")
        run_headless(school_world)
        return
    root.withdraw()
    ...
    btn = tk.Button(overlay, command=lambda: switch_world(school_world), bg="#caa54a", fg="black",
    ...
    def poll():
        ...
        target_ready = not (other and other.get("load_seq")) or jail_ready_for_current(school_world)
