...
## 13. GIT / REPO (NEU spaet 2026-06-24)

**Das Projekt ist jetzt versioniert auf GitHub:** https://github.com/i0am0lost/MWA2-0.001 (public, Tag `v0.001`).
- Lokales Git in `<project-root>`. Remote `clean` -> `MWA2-0.001`, Upstream `main` -> `clean/main`.
  **Kuenftige Commits/Pushes laufen autonom** (`git commit` + `git push`; GitHub-Creds sind im Windows-
  Credential-Manager gecacht). HINWEIS: der Token darf pushen, aber NICHT loeschen (`delete_repo` fehlt).
- **Altes Repo `i0am0lost/MWA2`** (erster Push enthielt versehentlich persoenliche Pfade/E-Mail) -> wird/ist
  auf **privat** gestellt. NICHT mehr verwenden; kanonisch = `MWA2-0.001`.
- **Im Repo ist NUR Projekt-Tooling** (Custom-Mods, Python, Docs) -> `.gitignore` schliesst die ~21 GB
  Spielordner `school/`+`jail/` und Saves aus. Ein frischer Clone braucht eine AA2/AAUnlimited-Installation,
  als `school/` und `jail/` neben die Dateien gelegt.
- **Pfade sind jetzt PORTABEL:** `orchestrator.py` & `_re/*.py` nutzen `_BASE = os.path.dirname(__file__)`
  statt hardcodierter `D:\...`-Pfade. Docs genericisiert (`<project-root>`, `python`). Lokal real:
  Projekt = der Ordner mit diesen Dateien; Python 3.12. `run_orchestrator.bat` nutzt `python`/`py` von PATH.
- **README ist ENGLISCH** (Landing-Page fuer die Community) und erklaert das Projekt als generische
  Multi-Welten-Erweiterung (jail = erste Instanz, vervielfaeltigbar). Detail-Docs bleiben Deutsch.

**WICHTIG fuer die naechste Session:** persoenliche Pfade (Username, E-Mail, lokale absolute Pfade) IMMER
genericisieren BEVOR etwas gepusht wird (Lektion aus diesem Versehen). Push-Ziel = `clean`/`MWA2-0.001`.
